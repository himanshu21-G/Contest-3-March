...
async function scrapeGithubTrending() {
  ...
  for (let repository of repositories) {
    ...
    const description = await repository.$eval('p', node => node.innerText.trim());
  ...
  // Extract data for developers
  const developers = await page.$$('ul.filter-list + div > div > article');
  const devResults = [];
  ...
  for (let developer of developers) {
    ...
    const repoDescription = await developer.$eval('h1 + p > a + span', node => node.innerText.trim());
